function [Y_hat,all_index,NumFea_hat,NumWin_hat,NumEnhan_hat] = ...
    bls_regression_Y_sub_noTest_2(train_x,train_y,Fea_vec,Win_vec,...
    Enhan_vec,validation_x,Vp_known,den_known,f,modes_num_vec,index_vec,index_vec_all,mean_y,std_y,validation_x_norm,h_known)
%
...
for Num_i= 1:1:length(Fea_vec) % searching range for hyperparameter 1
    for Num_j=1:1:length(Win_vec) % searching range for hyperparameter 2
        for Num_k=1:1:length(Enhan_vec) % range for hyperparameter 3
            ...
            [NetoutValidation,test_error_Y] = bls_train_Y_noTest_2(train_x,train_y,...
                validation_x,WF,WeightEnhan,s,C,NumFea,NumWin,Vp_known,den_known,f,modes_num_vec,index_vec,index_vec_all,mean_y,std_y,validation_x_norm,h_known);

            result = [result; NumFea NumWin NumEnhan test_error_Y NetoutValidation]; % recording all the searching reaults
